...
int main() {
	...
	while (!SS.IsGameEnd()) {
		...
		if (key == 'w') {
			SS.MoveHero(SoukoBanSystem::Direction::Down);
		}
		if (key == 's') {
			SS.MoveHero(SoukoBanSystem::Direction::Up);
		}
		if (key == 'a') {
			SS.MoveHero(SoukoBanSystem::Direction::Left);
		}
		if (key == 'd') {
			SS.MoveHero(SoukoBanSystem::Direction::Right);
		}
		std::cin.clear();
	}
	Show(SS);
